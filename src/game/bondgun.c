...
void bgunUpdateBlend(struct hand *hand, s32 handnum)
{
	...
	interpolate_cubic_spline(&hand->blendpos[(pos + 3) % 4], &hand->blendpos[pos], &hand->blendpos[(pos + 1) % 4], &hand->blendpos[(pos + 2) % 4], hand->dampt, &sp5c);
	interpolate_cubic_spline(&hand->blendlook[(pos + 3) % 4], &hand->blendlook[pos], &hand->blendlook[(pos + 1) % 4], &hand->blendlook[(pos + 2) % 4], hand->dampt, &sp50);
	interpolate_cubic_spline(&hand->blendup[(pos + 3) % 4], &hand->blendup[pos], &hand->blendup[(pos + 1) % 4], &hand->blendup[(pos + 2) % 4], hand->dampt, &sp44);
...
void bgunApplyVRControllerPos(struct hand *hand, s32 handnum, Mtxf *dest) {
	...
	if (connected) {
        ...
        float verta = g_Vars.currentplayer->vv_verta * (M_PI / 180.0f);
		...
		controllerMatrix.m[3][0] = controller_pos[0] + g_Vars.currentplayer->prop->pos.x;
		controllerMatrix.m[3][1] = controller_pos[1] + g_Vars.currentplayer->vv_manground;
		controllerMatrix.m[3][2] = controller_pos[2] + g_Vars.currentplayer->prop->pos.z;
